...
void main() {
  ...
  test('authenticated identities get separate local accounts and logout returns offline', () async {
    final alice = await db.localAccountDao.ensureAuthenticatedAccount(
      userId: 'auth-alice', email: 'alice@example.com', displayName: 'Alice', avatarUrl: null,
    );
    final bob = await db.localAccountDao.ensureAuthenticatedAccount(
      userId: 'auth-bob', email: 'bob@example.com', displayName: 'Bob', avatarUrl: null,
    );
  ...
  test('account activation gates scoped work and keeps outbox ownership isolated', () async {
    final alice = await db.localAccountDao.ensureAuthenticatedAccount(
      userId: 'gate-alice', email: 'alice@gate.test', displayName: 'Alice', avatarUrl: null,
    );
    await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Alice task', category: 'Test', dueDate: DateTime.now(), createdAt: DateTime.now(),
    ));
    final bob = await db.localAccountDao.ensureAuthenticatedAccount(
      userId: 'gate-bob', email: 'bob@gate.test', displayName: 'Bob', avatarUrl: null,
    );
    await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Bob task', category: 'Test', dueDate: DateTime.now(), createdAt: DateTime.now(),
    ));
    expect(db.activeAccountId, bob);
    expect((await db.syncOutboxDao.pendingCount(alice)), greaterThanOrEqualTo(2));
    expect((await db.syncOutboxDao.pendingCount(bob)), greaterThanOrEqualTo(2));
    await db.localAccountDao.activate(alice);
    expect(db.activeAccountId, alice);
    expect((await db.taskDao.watchAllTasks().first).single.title, 'Alice task');
    await db.localAccountDao.ensureOfflineAccount();
    expect((await db.localAccountDao.getActive())?.authProvider, 'offline');
  ...
  test('v9 finance types use minor units and no loan schema remains', () async {
    await db.moneyDao.insertTransaction(MoneyTransactionsCompanion.insert(
      type: 'lent', amount: 12550, category: 'Lent', date: DateTime.now(),
    ));
    await db.moneyDao.insertTransaction(MoneyTransactionsCompanion.insert(
      type: 'borrowed', amount: 20000, category: 'Borrowed', date: DateTime.now(),
    ));
    expect(await db.moneyDao.totalForType('lent'), 12550);
    expect(await db.moneyDao.totalForType('borrowed'), 20000);
    final columns = await db.customSelect('PRAGMA table_info(money_transactions)').get();
    expect(columns.map((row) => row.read<String>('name')), isNot(contains('loan_id')));
  });

  test('finance update returns bool while preserving Drift write semantics', () async {
    final id = await db.moneyDao.insertTransaction(MoneyTransactionsCompanion.insert(
      type: 'expense', amount: 100, category: 'Test', date: DateTime(2020),
    ));
    expect(await db.moneyDao.updateTransaction(MoneyTransactionsCompanion(
      id: Value(id), type: const Value('income'), amount: const Value(200),
      category: const Value('Updated'), date: Value(DateTime(2020, 1, 2)),
    )), isTrue);
    expect((await db.moneyDao.watchAllTransactions().first).single.type, 'income');
    expect(await db.moneyDao.updateTransaction(const MoneyTransactionsCompanion(id: Value(999999))), isFalse);
  });

  test('note update returns bool and ignores tombstoned rows', () async {
    final noteDao = NoteDao(db);
    final id = await noteDao.insertNote(NotesCompanion.insert(
      title: 'Original', content: 'Body', category: 'Test', createdAt: DateTime(2020), updatedAt: DateTime(2020),
    ));
    expect(await noteDao.updateNote(NotesCompanion(
      id: Value(id), title: const Value('Updated'), content: const Value('Body 2'), category: const Value('Test'),
    )), isTrue);
    expect((await noteDao.watchAllNotes().first).single.title, 'Updated');
    await noteDao.deleteNote(id);
    expect(await noteDao.updateNote(NotesCompanion(id: Value(id), title: const Value('No update'))), isFalse);
  });
  ...
  test('local mutations create one coalesced outbox operation', () async {
    final taskId = await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Outbox task', category: 'Test', dueDate: DateTime.now(), createdAt: DateTime.now(),
    ));
    expect(await db.syncOutboxDao.pendingCount(db.activeAccountId), 1);
    await db.taskDao.updateTask(TasksCompanion(id: Value(taskId), title: const Value('Updated')));
    expect(await db.syncOutboxDao.pendingCount(db.activeAccountId), 1);
    final row = await (db.select(db.syncOutbox)..where((o) => o.localRowId.equals(taskId))).getSingle();
    expect(row.operation, OutboxOperation.create);
    await db.taskDao.deleteTask(taskId);
    final deleted = await (db.select(db.syncOutbox)..where((o) => o.localRowId.equals(taskId))).getSingle();
  ...
  test('authenticated account and profile mutations are represented', () async {
    ...
    await db.profileDao.upsertProfile(ProfileTableCompanion(
      id: Value(accountId),
      name: const Value('Outbox User'),
      role: const Value('Student'),
    ));
    final rows = await (db.select(db.syncOutbox)..where((o) => o.localAccountId.equals(accountId))).get();
    expect(rows.map((row) => row.entityType), containsAll([OutboxEntity.account, OutboxEntity.profile]));
  });

  test('outbox preserves relationship order and retry/completion state', () async {
    final taskId = await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Parent', category: 'Test', dueDate: DateTime.now().add(const Duration(days: 1)), createdAt: DateTime.now(),
    ));
    final reminderId = await db.reminderDao.insertReminder(RemindersCompanion.insert(
      taskId: Value(taskId), title: 'Child', dueAt: DateTime.now().add(const Duration(days: 1)),
    ));
    final entries = await (db.select(db.syncOutbox)..where((o) => o.localAccountId.equals(db.activeAccountId))).get();
    final task = entries.singleWhere(
      (e) => e.localRowId == taskId && e.entityType == OutboxEntity.task,
    );
    final reminder = entries.singleWhere(
      (e) => e.localRowId == reminderId && e.entityType == OutboxEntity.reminder,
    );
    expect(task.dependencyRank, lessThan(reminder.dependencyRank));
    await db.syncOutboxDao.markProcessing(task.id);
    await db.syncOutboxDao.markRetryable(task.id, 'offline', delay: Duration.zero);
    final retry = await (db.select(db.syncOutbox)..where((o) => o.id.equals(task.id))).getSingle();
    expect(retry.state, OutboxState.retryableError);
    expect(retry.attemptCount, 1);
    await db.syncOutboxDao.markCompleted(task.id);
    expect((await (db.select(db.syncOutbox)..where((o) => o.id.equals(task.id))).getSingle()).state, OutboxState.completed);
  ...
  test('pending outbox survives database reopen', () async {
    ...
    await first.taskDao.insertTask(TasksCompanion.insert(
      title: 'Persistent outbox task', category: 'Test', dueDate: DateTime.now(), createdAt: DateTime.now(),
    ));
    await first.close();
  ...
  test('reminders link to planner tasks and delete transactionally', () async {
    final taskId = await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Future event', category: 'Study', dueDate: DateTime.now().add(const Duration(days: 2)),
      dueTime: const Value('10:00'), isPlannerEntry: const Value(true), createdAt: DateTime.now(),
    ));
    final reminderId = await db.reminderDao.insertReminder(RemindersCompanion.insert(
      taskId: Value(taskId), title: 'Future event', dueAt: DateTime.now().add(const Duration(days: 2)),
    ));
  ...
  test('heatmap query is bounded to requested planner range', () async {
    ...
    await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Old', category: 'Study', dueDate: DateTime(2020), isPlannerEntry: const Value(true), createdAt: now,
    ));
    await db.taskDao.insertTask(TasksCompanion.insert(
      title: 'Recent', category: 'Study', dueDate: now, isPlannerEntry: const Value(true), createdAt: now,
    ));
  ...
  test('youtube playlists and progress are isolated by account', () async {
    final playlistId = await db.youtubePlaylistDao.savePlaylist(
      playlist: YoutubePlaylistsCompanion.insert(
        youtubePlaylistId: 'PL1234567890123', title: 'A', createdAt: DateTime.now(), updatedAt: DateTime.now(),
      ),
      videos: [YoutubeVideosCompanion.insert(
        playlistLocalId: 0, youtubeVideoId: 'video-a', title: 'Video A', position: 0, createdAt: DateTime.now(), updatedAt: DateTime.now(),
      )],
    );
    ...
    expect((await db.youtubePlaylistDao.getVideos(playlistId)).single.completed, isTrue);
    final accountB = await db.into(db.localAccounts).insert(LocalAccountsCompanion.insert(
      authProvider: 'offline-test', displayName: const Value('B'), createdAt: DateTime.now(),
    ));
    await db.localAccountDao.activate(accountB);
  ...
  test('reminders carry the local account foreign key', () async {
    final foreignKeys = await db.customSelect('PRAGMA foreign_key_list(reminders)').get();
    expect(foreignKeys.map((row) => row.read<String>('table')), contains('local_accounts'));
  });

  test('phase 1 sync metadata is additive and detection data stays local', () async {
    const syncTables = [
      'local_accounts',
      'profile_data',
      'tasks',
    ...
    final eventColumns = await db.customSelect(
      'PRAGMA table_info(transaction_detection_events)',
    ).get();
    expect(
  ...
  test('task deletion tombstones the task and linked reminder', () async {
    ...
    expect(task.updatedAt.isAfter(before) || task.updatedAt.isAtSameMomentAs(before), isTrue);
    expect(reminder.serverId, 'reminder-server-id');
    expect(reminder.deletedAt, isNotNull);
    expect(await db.taskDao.watchPlannerEntriesInRange(DateTime(2019), DateTime(2030)).first, isEmpty);
    expect(await db.reminderDao.getByTaskId(taskId), isNull);
  });
  ...
  test('playlist deletion tombstones playlist and videos without losing progress fields', () async {
    ...
    final video = (await db.youtubePlaylistDao.getVideos(playlistId)).single;
    await db.youtubePlaylistDao.setCompleted(video.id, true);
    final before = await (db.select(db.youtubeVideos)..where((v) => v.id.equals(video.id))).getSingle();
    await db.youtubePlaylistDao.deletePlaylist(playlistId);

    final playlist = await (db.select(db.youtubePlaylists)..where((p) => p.id.equals(playlistId))).getSingle();
    final deletedVideo = await (db.select(db.youtubeVideos)..where((v) => v.id.equals(video.id))).getSingle();
