...
class _Remote implements SyncRemoteApi {
  ...
  Future<Map<String, dynamic>> insertRow(
    ...
    final row = {...payload, 'updated_at': '2026-09-23T10:00:00.123456Z'};
...
void main() {
  ...
  test(
    'completing a To-Do retains it, even with an old completion date',
    () async {
  ...
  test('completing a Planner event retains it in its historical range', () async {
    // Use a date in the future so _isPastPlannerDate does not block the toggle.
  ...
  test(
    'completion is pushed and remains after pull without becoming a tombstone',
    () async {
      final id = await db.taskDao.insertTask(
        TasksCompanion.insert(
      ...
      final localBefore = await (db.select(
        db.tasks,
      ...
      await service().syncNow();
      final localAfter = await (db.select(
        db.tasks,
      )..where((t) => t.id.equals(id))).getSingle();
