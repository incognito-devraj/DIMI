...
class TransactionDetectionService {
  ...
  static const _channel = MethodChannel(
    'com.dimi.dimi_app/transaction_detection',
  );
  ...
  Future<String> detectionMode() async =>
      await _channel.invokeMethod<String>('getDetectionMode') ?? 'Detect & Ask';
  ...
  Future<int> syncPendingEvents() async {
    ...
    final items =
        await _channel.invokeMethod<List<dynamic>>(
          'getPendingTransactionEvents',
        ) ??
        const [];
    var imported = 0;
    for (final item in items) {
      final map = Map<String, dynamic>.from(item as Map);
      final event = RawTransactionEvent(
        id: map['id'] as String,
        sourcePackage: map['sourcePackage'] as String? ?? '',
        sourceType: map['sourceType'] as String? ?? 'NOTIFICATION',
      ...
      await db.transactionDetectionDao.insertEvent(
      ...
      final recent = await db.transactionDetectionDao.recentCandidates(
        candidate.timestamp.subtract(const Duration(minutes: 10)),
      );
      final existing = recent.where((row) {
        final close =
            (row.occurredAt.difference(candidate.timestamp)).abs() <=
            const Duration(minutes: 10);
        final referenceMatch =
            candidate.referenceId != null &&
            row.referenceId == candidate.referenceId;
        final merchantMatch =
            row.merchantIdentity == candidate.merchantIdentity ||
            row.merchantIdentity == 'unknown' ||
            candidate.merchantIdentity == 'unknown';
        return referenceMatch ||
            (row.amountMinor == candidate.amountMinor &&
                row.direction == candidate.direction.name &&
                close &&
                merchantMatch);
      }).toList();
      final duplicate = existing.isNotEmpty;
      if (duplicate) {
        final primary = existing.firstWhere(
          (row) => row.status != 'DUPLICATE',
          orElse: () => existing.first,
        );
        await db.transactionDetectionDao.enrichCandidate(
          candidateId: primary.candidateId,
          source: candidate.source,
          referenceId: candidate.referenceId,
          accountHint: candidate.accountHint,
          paymentMethod: candidate.paymentMethod,
          balanceAfterMinor: candidate.balanceAfterMinor,
        );
      ...
      final auto =
          mode == 'Auto-add high confidence' && candidate.confidence >= 0.8;
      await db.transactionDetectionDao.insertCandidate(
        TransactionCandidatesCompanion.insert(
          candidateId: candidate.id,
          amountMinor: candidate.amountMinor,
          merchantName: Value(candidate.merchantName),
          merchantIdentity: Value(candidate.merchantIdentity),
          direction: candidate.direction.name.toUpperCase(),
          transactionType: candidate.type.name.toUpperCase(),
          source: candidate.source,
          bankConfirmationStatus: Value(
            candidate.source.contains('BANK') ? 'RECEIVED' : 'NOT_RECEIVED',
          ),
          sourcePackage: Value(candidate.sourcePackage),
          occurredAt: candidate.timestamp,
          referenceId: Value(candidate.referenceId),
          accountHint: Value(candidate.accountHint),
          paymentMethod: Value(candidate.paymentMethod),
          balanceAfterMinor: Value(candidate.balanceAfterMinor),
          rawEventId: Value(candidate.rawEventId),
  ...
  Future<void> _ack(String id) =>
      _channel.invokeMethod<void>('acknowledgeTransactionEvent', {'id': id});
}
