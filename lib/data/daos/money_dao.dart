...
class MoneyDao extends DatabaseAccessor<AppDatabase> with _$MoneyDaoMixin {
  ...
  Stream<List<MoneyTransaction>> watchAllTransactions() =>
    ...
    return (select(moneyTransactions)
          ..where(
            (t) =>
                t.localAccountId.equals(db.activeAccountId) &
                t.deletedAt.isNull() &
                t.date.isBetweenValues(weekStart, weekEnd),
          )
          ..orderBy([(t) => OrderingTerm.desc(t.date)]))
        .watch();
  }
  ...
  Future<int> insertTransaction(MoneyTransactionsCompanion entry) async {
    final id = await into(moneyTransactions)
  ...
  Future<bool> updateTransaction(MoneyTransactionsCompanion entry) async {
    ...
    final changed =
        await (update(moneyTransactions)..where(
              (t) =>
                  t.id.equals(entry.id.value) &
                  t.localAccountId.equals(db.activeAccountId) &
                  t.deletedAt.isNull(),
